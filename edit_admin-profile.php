...
$row = mysqli_fetch_assoc($res);
   ...
   if($oldname!=$name){
    ...
    if(!isset($error)){ 
          if($file!= "")
          {
            $stmt = mysqli_query($conn,"SELECT image FROM  admins WHERE email='$email'");
            $row = mysqli_fetch_array($stmt); 
            $deleteimage=$row['admin_pp'];
            unlink($folder.$deleteimage);
            move_uploaded_file($file, $folder . $new_image_name); 
            mysqli_query($conn,"UPDATE admins SET image='$new_image_name' WHERE email='$email'");
          }
           $result = mysqli_query($conn,"UPDATE admins SET name='name' WHERE email='$email'");
           if($result)
           {
       header("location:admin_profile.php?profile_updated=1");
           }
           else 
           {
